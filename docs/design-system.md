...
This pattern describes how a single entity (like a **Team** or **Badge Collection**) can exist in multiple contexts while maintaining a single source of truth.

- **Mechanism**:
    - **Sharing**: The owner of an item (e.g., a Team) can share it with all other users by dragging its card from the main management board and dropping it into the "Shared Items" side panel. This action sets an `isShared` flag on the item but **does not remove it from the owner's board**.
