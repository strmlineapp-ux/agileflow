...
function BadgeDisplayItem({ badge, viewMode, onUpdateBadge, onDelete, collectionId, teamId, isSharedPreview = false }: { badge: Badge; viewMode: BadgeCollection['viewMode']; onUpdateBadge: (badgeData: Partial<Badge>) => void; onDelete: () => void; collectionId: string; teamId: string; isSharedPreview?: boolean; }) {
    ...
    const sharedIconAndColorEditor = (
        <div className="relative">
             ...
             <Popover open={isColorPopoverOpen} onOpenChange={setIsColorPopoverOpen}>
                <PopoverTrigger asChild disabled={!isEditable}>
                    {tooltipWrapper(!isEditable, "Properties are managed by the owner team.", (
                        <div className={cn("absolute -bottom-0.5 -right-0.5 h-4 w-4 rounded-full border-2 border-muted", isEditable ? "cursor-pointer" : "cursor-not-allowed")} style={{ backgroundColor: badge.color }} />
                    ))}
                </PopoverTrigger>
                <PopoverContent className="w-auto p-2">
                <div className="grid grid-cols-8 gap-1">{predefinedColors.map(c => (<button key={c} className="h-6 w-6 rounded-full border" style={{ backgroundColor: c }} onClick={() => {onUpdateBadge({ color: c }); setIsColorPopoverOpen(false);}}/>))}<div className="relative h-6 w-6 rounded-full border flex items-center justify-center bg-muted"><GoogleSymbol name="colorize" className="text-muted-foreground" /><Input type="color" value={badge.color} onChange={(e) => onUpdateBadge({ color: e.target.value })} className="absolute inset-0 h-full w-full cursor-pointer opacity-0 p-0"/></div></div>
    ...
    const inlineNameEditor = isEditingName && isEditable ? (
      ...
      tooltipWrapper(!isEditable, "Properties are managed by the owner team.", (
        <span
            className={cn("font-semibold break-words", isEditable && "cursor-text", viewMode === 'assorted' && 'text-xs')}
            onClick={(e) => {
            e.stopPropagation();
            if (isEditable) {
    ...
    // Assorted View
    return (
        <div className="group relative inline-flex items-center gap-1 rounded-full border-2 py-0.5 px-2" style={{ borderColor: badge.color, color: badge.color }}>
            <div className="relative flex items-center justify-center">
                <GoogleSymbol name={badge.icon} style={{color: badge.color}} className="text-xs" />
                {shareIcon && (
                    <TooltipProvider>
                        <Tooltip>
                            <TooltipTrigger asChild>
            ...
            {inlineNameEditor}
            {!isSharedPreview && (
                <button type="button" onClick={onDelete} className="ml-1 h-4 w-4 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-full inline-flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" aria-label={`Delete ${badge.name}`}>
...
function BadgeCollectionCard({ collection, allBadgesInTeam, teamId, teams, onUpdateCollection, onDeleteCollection, onAddBadge, onUpdateBadge, onDeleteBadge, onToggleShare, dragHandleProps, isSharedPreview = false }: {
            ...
            <div {...dragHandleProps}>
                <CardHeader>
                    <div className="flex items-start justify-between">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                            <div className="relative">
                                ...
                                />
                                <Popover open={isColorPopoverOpen} onOpenChange={setIsColorPopoverOpen}>
                                    <PopoverTrigger asChild disabled={!isOwned || isSharedPreview}><div className={cn("absolute -bottom-1 -right-0 h-4 w-4 rounded-full border-2 border-card", !isOwned || isSharedPreview ? "cursor-not-allowed" : "cursor-pointer")} style={{ backgroundColor: collection.color }} /></PopoverTrigger>
                                    <PopoverContent className="w-auto p-2">
                    ...
                    <div className="flex items-center justify-between pt-2">
                                ...
                                <StrictModeDroppable droppableId={`duplicate-badge-zone:${collection.id}`} type="badge" isDropDisabled={!isOwned}>
                                    {(provided, snapshot) => (
                                        <div
                                            ...
                                            className={cn("rounded-full p-0.5", snapshot.isDraggingOver && "ring-1 ring-border ring-inset")}
                                        >
                                            <TooltipProvider>
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                        ...
                        <div className="flex items-center gap-1">
                            {isOwned && !isSharedPreview && APPLICATIONS.map(app => (
                         ...
                         <div
                            ...
                            {collectionBadges.map((badge, index) => (
                                <Draggable key={`${badge.id}::${collection.id}`} draggableId={`${badge.id}::${collection.id}`} index={index} isDragDisabled={false}>
                                    {(provided, snapshot) => (<div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className={cn(snapshot.isDragging && "opacity-80")}>
                                        <BadgeDisplayItem 
                                            badge={badge} 
...
export function BadgeManagement({ team, tab }: { team: Team, tab: AppTab }) {
    ...
    const onDragEnd = (result: DropResult) => {
        ...
        if (type === 'collection' && source.droppableId === 'shared-collections-panel' && destination.droppableId === 'collections-list') {
            const collectionToDuplicate = sharedCollectionsFromOthers.find(c => c.id === draggableId);
            if (collectionToDuplicate) {
                const newCollectionId = crypto.randomUUID();
                
                const newBadges: Badge[] = (collectionToDuplicate.badgeIds || [])
                    .map(badgeId => {
                        const originalBadge = allBadgesAvailableToTeam.find(b => b.id === badgeId);
                        if (!originalBadge) return null;
                        return {
                            ...JSON.parse(JSON.stringify(originalBadge)),
                            id: crypto.randomUUID(),
                ...
                const newCollection: BadgeCollection = {
                    ...JSON.parse(JSON.stringify(collectionToDuplicate)),
                    id: newCollectionId,
                    ownerTeamId: team.id,
                    isShared: false,
                ...
                toast({ title: 'Collection Copied', description: `A copy of "${collectionToDuplicate.name}" was created for your team.` });
            }
            return;
        }
                    ...
                    <div className={cn(
                        ...
                        <Card className={cn("transition-opacity duration-300", isSharedPanelOpen ? "opacity-100" : "opacity-0")}>
                            <CardHeader>
                                ...
                                <CardDescription>Drag a collection to copy it to your team.</CardDescription>
                            </CardHeader>
                            <CardContent>
