...
function EventTemplateContent({ team }: { team: Team }) {
...
export function EventTemplateManagement({ team: teamFromProps, tab, page }: { team?: Team | null; tab: AppTab; page: AppPage }) {
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        />
        {selectedTeam ? (
            <EventTemplateContent team={selectedTeam} />
        ) : (
            <TeamSelection
                teams={teams}
                onSelectTeam={(team) => setSelectedTeam(team)}
                message="Select a team to manage its event presets."
            />
        )}
      </div>
    );
