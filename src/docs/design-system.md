...
-   **Initiating a Drag (Hold to Drag)**: The drag action is initiated by clicking and holding anywhere on the card. The `PointerSensor` for `dnd-kit` **must** be configured with a `delay` (e.g., 250ms) across the entire application. This allows the library to intelligently distinguish a quick tap (for a click) from an intentional "hold-to-drag" action, creating a smooth and intuitive user experience.
...
-   **Visual Feedback & Drag Overlay**:
    -   **Standard (In-place)**: Visual changes (like a `shadow` or `opacity`) should be applied directly to the inner component based on the `isDragging` prop from `dnd-kit`'s `useSortable` hook. The draggable wrapper itself should remain untouched.
    -   **Drag Overlay (Ghost Card)**: A `DragOverlay` **must** be used to render a "ghost" of the card being dragged. This overlay has strict visual requirements:
        1.  **Always Collapsed**: The card inside the overlay must be rendered in its collapsed, header-only state. This is achieved by passing a prop like `isCollapsed={true}` to the card component when it's rendered within the overlay.
        2.  **Non-Interactive**: The overlay version of the card **must** be stripped of all interactive elements. This includes delete buttons, expand/collapse accordions, popover triggers, and any other action icons. The overlay is purely a visual representation of what is being dragged.
