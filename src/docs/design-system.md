...
-   **Initiating a Drag**: To provide a clean interface, the drag action is initiated by clicking and dragging any non-interactive part of the card. A `delay` of 250ms is used to differentiate between a click and a drag.
    -   **Complex Item Dragging**: For draggable items that contain multiple interactive elements (like the Badges within a Badge Collection), the standard click-and-hold can be unreliable. In these cases, the **required** pattern is to use a modifier key. The drag action **must** be configured to only initiate when the user-defined modifier key (defaulting to `Shift`) is held down while clicking. This disambiguates the user's intent, allowing for quick clicks to trigger interactions (like opening an icon picker) and `Modifier+Click+Drag` to move the item.
