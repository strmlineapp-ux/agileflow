...
-   **Initiating a Drag**: The default drag trigger is a click-and-hold for 250ms anywhere on the non-interactive parts of the card.
    -   **`Alt+Click` Dragging for Complex Items**: For draggable items that contain multiple interactive elements (like the Badges within a Badge Collection), the standard click-and-hold can be unreliable. In these cases, the **required** pattern is to use a modifier key. The drag action **must** be configured to only initiate when the `Alt` key is held down while clicking. This disambiguates the user's intent, allowing for quick clicks to trigger interactions (like opening an icon picker) and `Alt+Click+Drag` to move the item.
