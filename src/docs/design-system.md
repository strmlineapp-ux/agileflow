...
-   **Preventing Interaction Conflicts (CRITICAL)**: To allow interactive elements like an **Inline Editor** to function correctly inside a draggable card without triggering the drag action, the `useSortable` hook **must** be disabled while an edit is active.
    -   **State Management**: The component that calls `useSortable` (the "Sortable Parent") **must** own the `isEditing` state. For components with multiple editable fields (like a Badge name and description), the Sortable Parent must own a state for each (e.g., `isEditingName`, `isEditingDescription`).
    -   **State Propagation**: The state and its setter function must be passed down as props to the child component containing the input fields.
    -   **Disabling Drag**: The `useSortable` hook in the Sortable Parent must be configured with `disabled: isEditingName || isEditingDescription`.
    -   **Effect**: When a user clicks to edit a field, the child component calls the setter function passed down from the parent. This updates the state in the Sortable Parent, which immediately disables the `useSortable` hook. This prevents `@dnd-kit` from capturing pointer or keyboard events, allowing the inline editor to receive focus and input correctly without triggering an unwanted drag.
