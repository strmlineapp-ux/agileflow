...
-   **Drag-Ready State**: When the drag modifier key is held down, the application enters a "drag-ready" state to provide clear visual feedback and prevent accidental actions.
    - **Hide Interactive Elements**: All secondary interactive elements within draggable cards—such as delete buttons, popover triggers (except for the main entity icon), and expand/collapse icons—**must be hidden**. This is typically achieved by adding a `.hidden` class based on a global `isDragModifierPressed` state from the `UserContext`. The main icon remains visible but its popover trigger is disabled.
