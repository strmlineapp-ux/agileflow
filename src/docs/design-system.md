...
-   **Layout**: Entities are presented in a responsive grid of cards. To ensure stability during drag operations, especially across multiple rows, the container must use a `flex flex-wrap` layout instead of CSS Grid. Each draggable card item is then given a `basis` property (e.g., `basis-full md:basis-[calc(50%-1.5rem)]`) to create the responsive columns. **Crucially, `flex-grow` should be avoided on these items**, as it can cause the grid to reflow unstably when an item is being dragged.
