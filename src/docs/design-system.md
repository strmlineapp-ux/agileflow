...
-   **Preventing Interaction Conflicts**:
    -   **Click vs. Drag**: To allow interactive elements inside a draggable item (like popover triggers or inline editors) to function on a quick click without initiating a drag, the `PointerSensor` for `dnd-kit` must be configured with a `delay` (e.g., 250ms). This allows the library to distinguish a quick tap from an intentional "hold-to-drag" action. This is the preferred method over stopping event propagation, as it allows the entire item to feel draggable.
    -   **Direct Action Buttons (e.g., Delete)**: For buttons that trigger an immediate, critical action (like "Delete"), relying solely on the pointer delay can sometimes feel unresponsive or ambiguous. To ensure these buttons fire instantly and reliably on a click, they must be programmatically isolated from the drag listener. This is achieved by adding `onPointerDown={(e) => e.stopPropagation()}` directly to the button component. This is a crucial exception to the general rule and ensures high-priority actions are never missed.
    -   **Editing State**: To prevent conflicts with keyboard interactions (e.g., using the spacebar in an **Inline Editor** vs. spacebar to lift a draggable item), the `useSortable` hook for the draggable item **must** be temporarily disabled while any internal component is in an editing state. This is done by passing a `disabled: isEditing` flag to the hook, preventing `@dnd-kit`'s keyboard listeners from firing while the user is typing.
