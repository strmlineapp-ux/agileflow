...
-   **Initiating a Drag**: To provide a clean interface, the drag action is initiated by clicking and dragging any non-interactive part of the card. The drag listener from `dnd-kit`'s `useSortable` hook is applied to the main card container. The `PointerSensor` should be configured with a `delay` to differentiate between a quick click and an intentional long-press to drag.
...
-   **Preventing Interaction Conflicts**:
    -   **Click vs. Drag**: The `PointerSensor` for `dnd-kit` must be configured with a `delay` (e.g., 250ms). This allows the library to distinguish a quick tap (for a click) from a "hold-to-drag" action. This is the preferred method over stopping event propagation, as it allows the entire item to feel draggable while still permitting quick clicks on interactive elements within it.
    -   **Direct Action Buttons (e.g., Delete)**: For buttons that trigger an immediate, critical action (like "Delete"), relying on the pointer delay is not sufficient. To ensure these buttons fire instantly and reliably on a click, they must be programmatically isolated from the drag listener. This is achieved by adding `onPointerDown={(e) => e.stopPropagation()}` directly to the button component. This is a crucial exception to the general rule and ensures high-priority actions are never missed.
    -   **Inline Editors**: To prevent conflicts with keyboard interactions (e.g., using the spacebar in an **Inline Editor** vs. spacebar to lift a draggable item), the `useSortable` hook for the draggable item **must** be temporarily disabled while any internal component is in an editing state.
        - **State Management**: The component that implements the `useSortable` hook (e.g., `SortablePageCard`, `SortableBadgeItem`, `SortableCollectionCard`) must own the `isEditing` state.
        ...
        - **Disabling Drag**: The `useSortable` hook must be configured with `disabled: isEditing`. This is the key to resolving the "ghosting" effect when an editor is clicked.
        - **Effect**: When a user clicks to edit, the child component sets the parent's state to `true`. This immediately disables the `useSortable` hook, preventing `@dnd-kit` from capturing pointer or keyboard events, which allows the inline editor to function correctly without triggering a drag.
