...
-   **Initiating a Drag**: To provide a clean interface, the drag action is initiated by clicking and dragging any non-interactive part of the card, or by using the keyboard.
    -   **Modifier Key**: To avoid accidental drags on complex components with many internal interactive elements (like Badges), the drag-and-drop interaction **must** be initiated by holding down a user-configurable modifier key (`Shift`, `Alt`, `Ctrl`, or `Meta`) while clicking.
