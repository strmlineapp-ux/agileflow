...
- **Set Modifier Key**: Click the keyboard icon to set a custom modifier key (`Shift`, `Alt`, `Control`, or `Meta`) that must be held down to trigger secondary actions, like resetting a preference. The tooltip will show the currently selected key.
