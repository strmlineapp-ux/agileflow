...
- **Admin Groups**: Create and manage high-level administrative groups. To assign users, you can either click the `add_circle` button on a group's card, or simply drag a user card and drop it into another group to add them. Dropping a user onto the 'Admins' card will promote them to a full Admin while keeping their existing group memberships.
